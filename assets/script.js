const translations = {
  en: {
    ...
    'hero.text': 'Senior Unity Developer with 5+ years of commercial experience building gameplay architecture, multiplayer and UI/meta systems, performance tooling and build automation for PC, console and mobile games.',
    'hero.emailCta': 'Email',
    ...
    'cases.loadingProblem': 'Hub and mission loads pulled oversized dependencies, while enemies, loot and interactables were created before the player needed them.',
    'cases.loadingSolution': 'Separated scene and shared content, reorganized Addressables packing and deferred non-critical objects until they were needed near the player.',
    ...
    'cases.memoryProblem': 'Completed mission data accumulated between runs because static events, closures, global services and an undisposed ECS context retained the mission object graph.',
    'cases.memorySolution': 'Used Memory Profiler to trace retention, fixed subscriptions and cleanup, disposed the mission context and removed duplicated bundled content.',
    ...
    'cases.performanceProblem': 'Ability simulation repeated evaluations and allocations; enemy turns recalculated abilities and paths, causing severe stalls.',
    'cases.performanceSolution': 'Removed duplicate calculations, optimized pathfinding and replaced temporary collections with custom thread-safe pools.',
    ...
    'cases.buildProblem': 'Production builds contained duplicated assets and shaders, while bootstrap dependencies pulled unnecessary content into the initial package.',
    'cases.buildSolution': 'Audited the Addressables Build Report, removed duplicates, separated bootstrap dependencies and excluded unused shaders.',
    'cases.buildResult': 'Reduced the production build from 23 GB to 12 GB (-48%).',
    'cases.deliveryProblem': 'Publishing builds, preparing download pages and notifying the team required repeated manual work.',
    'cases.deliverySolution': 'Connected TeamCity, Cloudflare R2, n8n and Discord into one automated delivery workflow.',
    'cases.deliveryResult': 'Build delivery no longer required manual uploads, link preparation or team notifications.',
  ...
  ru: {
    ...
    'hero.eyebrow': 'Senior Unity Developer',
    'hero.title': 'Kenan Ibragimov',
    'hero.text': 'Senior Unity Developer с опытом более 5 лет: gameplay-архитектура, multiplayer и UI/meta-системы, оптимизация, внутренние инструменты и автоматизация билдов для ПК, консолей и мобильных игр.',
    'hero.emailCta': 'Почта',
    'hero.pdfCta': 'Скачать CV',
    'hero.casesCta': 'Посмотреть достижения',
    ...
    'cases.loadingProblem': 'Хаб и миссии подтягивали избыточные зависимости, а враги, лут и интерактивные объекты создавались раньше необходимости.',
    'cases.loadingSolution': 'Разделил контент сцен и общие ресурсы, перестроил упаковку Addressables и отложил создание второстепенных объектов.',
    ...
    'cases.memoryProblem': 'Данные миссий накапливались между запусками: их удерживали статические события, замыкания, глобальные сервисы и ECS-контекст.',
    'cases.memorySolution': 'Через Memory Profiler нашёл удержания, исправил подписки и очистку, уничтожил контекст и удалил дубликаты контента.',
    'cases.memoryResult': 'Хаб: с 6 до 3 ГБ; миссии: с 7-10 до 4,5-5,3 ГБ.',
    'cases.performanceTitle': 'Стабилизация FPS',
    'cases.performanceProblem': 'Симуляция способностей повторяла расчёты и аллокации, а ходы противника заново оценивали действия и пути.',
    'cases.performanceSolution': 'Убрал повторы, оптимизировал pathfinding и заменил временные коллекции потокобезопасными пулами.',
    ...
    'cases.buildProblem': 'В билд попадали дубликаты ассетов и шейдеров, а bootstrap-зависимости добавляли лишний контент.',
    'cases.buildSolution': 'Проверил Addressables Build Report, удалил дубликаты, отделил bootstrap-зависимости и исключил лишние шейдеры.',
    'cases.buildResult': 'Сократил размер билда с 23 до 12 ГБ (-48%).',
    'cases.deliveryProblem': 'Публикация билдов, страницы загрузки и уведомления команды требовали ручной работы.',
    'cases.deliverySolution': 'Объединил TeamCity, Cloudflare R2, n8n и Discord в единый автоматический workflow.',
    'cases.deliveryResult': 'Доставка билдов больше не требует ручных загрузок, подготовки ссылок и уведомлений.',
